...
- 类型分支-批量判断空接口中变量的类型
  ...
  ```
## 包
- go 语言中，类，函数，字段等元素都有访问的作用域，是私有还是公共的
- 以上元素的首字母大写代表可以被其他的包的代码引用
...
- 协程 goroutine
  ...
  ```
- 通道
  - 并发执行的函数之前需要交换数据，交换的载体就是通道，这里值不同协程通信的方法
  - 任何时候，同时只能有一个协程访问通道进行读写
  - 通道类似队列，需要声明通道收发的数据类型，先进先出，保证数据的顺序
  - 通道的创建与收发数据
  ```
  func main() {
      channel := make(chan int)   //创建一个通道
      go func() {   //通过协程开启一个匿名函数
        channel <- 0    //往通道发送数据
      }()
      result <- channel   //从通道接受数据
  }
  ```
