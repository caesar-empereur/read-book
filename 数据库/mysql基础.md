...
- **[mysql 各种日志文件](#)**
   - redolog
      - mysql数据写入后不是直接刷新到磁盘，而是刷新到内存缓冲区
      - mysql宕机后，内存的数据会丢失，这样无法保证数据的持久化
      - 因此为了保证内存中的数据不丢失，需要记录内存的数据修改了什么，redolog就是记录这些
      - redolog 是用于mysql宕机后根据这个日志找出内存中改了哪些东西,确保刷新到磁盘
   - undolog
      - mysql 的事务有回滚功能，就是undolog实现的
      - undolog 记录的是语句的逻辑日志，包括前后修改的值
      - 回滚的时候根据这些日志，生成逆向的语句来执行，实现回滚
      - mvcc 的多版本控制也是undolog实现的
      - mvcc 实际就是一条数据的多个版本，多个版本不是记录多条数据，而是根据undolog的节点计算出来

   - binlog
      - 是mysql写入的日志文件，有 statement, row, mixed 等几种格式
...
- mysql binlog 日志恢复数据
    ...
    - 转储binlog 为 sql 文件 mysqlbinlog --no-defaults D:\dev\app\mysql\mysql-5.7.33-winx64\data\build-binlog.000001 > D:\build-binlog.sql
