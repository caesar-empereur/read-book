...
- 过期删除对 RDB, AOF 文件的影响
    * SAVE 命令生成 RDB 文件时，过期的键被检查到，不会写到 RDB 文件中
    * redis 以 主 模式载入 RDB 时，过期的键不会载入
    * redis 以 从 模式载入 RDB 时，过期键也会载入，等到主删除过期键时，会发送同步命令删除从的
    * redis 以 AOF 模式运行时，过期键还没删除的也会写入 AOF, 只有过期时才向 AOF 文件追加 删除命令
    * 主 发现并删除过期键时，会同步删除命令到从
...
## 11章 AOF持久化
   ...
   * AOF 重写
      * 对于一个列表类型的键的 **多次写操作记录多条写命令**，AOF文件体积会很大，存储载入都耗资源
      * 对于一个列表类型的键，只需要一条写命令记录全部数据
      * 后台获取键对应的数据后生成一条命令，写入2个缓冲区，后续键的数据有变化时
      * 每一条写命令，后台会写入到 AOF **[缓冲区](#缓冲区)** 与 **[AOF重写缓冲区](#AOF重写缓冲区)**
      * 两个缓冲区的数据都会写入到对应的文件，重写结束时，重写缓冲区文件替换掉 缓冲区文件
...
```
-
    * 4.1 redis-trib对目标节点发送 导入哪个源节点的哪些 槽
    * 4.2 redis-trib对源节点发送 导出哪些槽到哪个目标节点
    * 4.3 redis-trib对源节点发送 导出哪些槽对应的 key 数据
    * 4.5 redis-trib对源节点发送 迁移哪些槽对应的 key 数据 到目标节点
    * 4.6 通知整个集群 哪些槽与数据迁移到哪里
    
-
    删除策略     | redis-trib 操作       | 键的数据结构   |
    ------------|--------------------|-------------------|
    源节点 |  ---->导入哪些槽               |  目标节点  |
    源节点 |  导出哪些槽-----> | 目标节点   |
    源节点 |  迁移槽对应的键数据------>           |  目标节点  |
    源节点 |  通知整个集群 : 哪些槽被迁移到哪里      |  目标节点  |
