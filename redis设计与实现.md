...
- 3 在集群中执行命令 （虽然是集群模式，但是执行命令时仍然需要指定哪个节点处理）
    ...
    * 3.1 某个节点接收到数据处理命令时，先算出 key 属于哪个槽
    * 3.2 判断该槽是不是自己负责的，是的就处理数据
    * 3.3 不是自己负责的，找到槽对应的节点，返回客户端 **[MOVED](#MOVED)** 错误 和 **[重定向](#重定向)** 指令
    * 3.4 **集群模式下客户端不会打印这个 MOVED 错误**，而是直接重定向到指定服务器 操作数据
    * 3.5 单机模式下，判断到key所属的槽不是自己的，客户端直接打印错误
