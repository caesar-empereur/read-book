...
## 11章 AOF持久化
   ...
   * **[AOF重写](#AOF重写)**
     ...
     |3   | 每一条写命令，后台会写入到 AOF **[缓冲区](#缓冲区)** 与 **[AOF重写缓冲区](#AOF重写缓冲区)** |
...
>> redis 没有采用一致性哈希，而是用哈希槽的方式，一致性哈希的方式如下
...
- 5 **[复制](#复制)** 与 **[故障转移](#故障转移)**
    ...
    * 5.2 其中 **[主节点用于处理槽](#主节点用于处理槽)**，**[而从节点则用于复制某个主节点](#而从节点则用于复制某个主节点)**
...
| 问题        | 描述 |解决方案|
|-----------|-------------------|-------------------|
|缓存穿透 | 缓存没有查到数据就查数据库，大量的请求没有命中缓存而转到数据库，给数据库造成大的压力 |数据库查到的空的值也缓存起来，之后再访问也会命中缓存 |
|缓存雪崩 |缓存挂掉导致所有请求转到数据库，造成数据库压力过大 |redis高可用(sentinel), 热点数据预热 |
|缓存击穿 | 热点key在某个时间内失效，有大量并发请求过来了，造成DB压力 | key获取value值为空时锁上，从数据库中load数据后再释放锁，其他线程等待锁释放 |
...
## redis 其他问题
   ...
   * redis 分布式锁的需要注意的问题
      * 设置 setnx 时，需要加上过期时间，防止锁释放失败或者忘记释放
