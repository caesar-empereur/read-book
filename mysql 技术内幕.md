...
## 第 4 章 mysql 数据文件
  ...
  * 表都是根据主键顺序组织存放的，这种存储方式的表称为 **[索引组织表](#索引组织表)**
  * 页 是mysql 磁盘上最小单位的数据存储形式，也叫 块 **[每页的数据大小是16KB](#每页的数据大小是16KB)**
...
>> 辅助索引的先通过找到聚集索引的键再查到到对应行记录的过程，出现了多次IO操作，这个叫**[回表查询](#回表查询)**,  但不是所有辅助索引都要经过回表查询
   ，覆盖索引不需要回表
...
>>**[mysql本身是支持分布式事务的，是由 XA 事务实现的](#mysql本身是支持分布式事务的，是由 XA 事务实现的)**
