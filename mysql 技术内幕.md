 ...
 * 事务的特性
  ...
  * 隔离级别与脏读幻读的关系
    ...
    |读的种类|种类描述|'|解决办法|办法描述
    |-----------|-------------------|-------------|-------------|-------------|
    |**[脏读(读未提交)](#脏读(读未提交))**|一个事务可以读取到另一个事务**未提交**|的数据|**[读提交](#读提交)**|一个事务必须等另一个事务提交后才能取|
    |**[不可重复读(虚读)](#不可重复读(虚读))**|一个事务可以读取到另一个事务**提交**的|**[更新数据](#更新数据)**|**[重复读](#重复读)**|一个读的事务提交后，不允许更新操作|
    |**[幻读](#幻读)**|一个事务可以读取到另一个事务**提交**的|**[插入数据](#插入数据)**|序列化|最高级别，事务按照串行执行，安全性最高|
